...
Data story site can be found at [site](https://valstrl.github.io/ADA_Data_Story/).

Code for data story site can be found at [site](https://github.com/valstrl/ADA_Data_Story).

# Contributions

- Kiran Bacsa : Data Visualization in Folium, Spectral Clustering, Scrapping/Handling TopoJson, Assisting Jekyll development, Writing Report
- Samuel Beuret : Genetic algorithm (coding and results), Adjacency Municipality Graph for geographical constraints, Scrapping Electoral Data, Generating Parliamentary Jsons
- Valentine Santarelli : Creating Jekyll website, Data Visualization HTML/CSS/Javascript, Scrapping Adjacency Graph, Data Cleaning (merging fusions, etc...)
...
do not take into account the evolutions of the Swiss society that occured in the last 150 years. Furthermore, since the number of seats the parliament is 
limited, some parties will never get enough votes to be represented, therefore part of the Swiss population is de facto excluded from parliamentary debates.
...
new Cantons. These new Cantons would then exhibit the best political representation of Switzerland. We would like to study the influence of the political 
partitioning on the parliament's composition. To this effect, we will tweak the different parameters of our model to study its effect. We can also verify 
...
- Is it possible to optimize both at the same time ?
...
The results for the municipalities that shared a common ballot box are registered among one of them, the other one have their number of votes set to zero. Since there is no mapping of the municipalities with the shared ballots, we could not copy their results and we kept the municipalities as such. They will still be clustered using only their geographical features.
